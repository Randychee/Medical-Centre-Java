            ...
            try (BufferedReader br = Files.newBufferedReader(PAYMENT_PATH)) {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    // Always show in table
                     String payId   = f.get(COL_PAY_ID).trim();
            String apptId  = f.get(COL_APPT_ID).trim();
            String dateStr = f.get(COL_DATE).trim(); // may be ""
            String timeStr = f.get(COL_TIME).trim(); // may be ""
            String custId  = f.get(COL_CUSTOMER).trim();

                     boolean isPaid = !dateStr.isEmpty() && !timeStr.isEmpty();
            if (!isPaid) unpaidCount++;

            // Always add the row to the table exactly as stored (show blanks for unpaid)
            tableModel.addRow(new Object[] { payId, apptId, amt, dateStr, timeStr, custId });

            // Only contribute to weekly revenue if it's a paid row in the current week window
            if (isPaid) {
                try {
...
private static java.util.Optional<java.time.LocalDate> latestDateAcrossFiles(java.nio.file.Path... paths) {
    ...
    private void initComponents() {
        ...
        appointmentProgress.setForeground(new java.awt.Color(51, 255, 0));
